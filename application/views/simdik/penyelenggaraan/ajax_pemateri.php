...
<tr class="tr_widyaiswara">
    ...
    <td>: <?php echo form_dropdown('jenis_pembicara', $pil,$q['jenis'], 'id="p1" class="jenis"') ?> <input type="text" class="nama_pmbcr" name="nama_pmbcr" value="<?php echo $q['nama']?>"/><input type="hidden" name="id_pmbcr[]" value="<?php echo $q['id_pembicara']?>"/> <span class="del_pmbcr btn btn-mini btn-danger">Hapus</span></td>
    <?php } else {?>
    <td>: <?php echo form_dropdown('jenis_pembicara', $pil,$q['jenis'], 'id="p1" class="jenis"') ?> <input type="text" class="nama_pmbcr" name="nama_pmbcr" value="<?php echo $q['nama']?>"/><input type="hidden" name="id_pmbcr[]" value="<?php echo $q['id_pembicara']?>"/> <span class="add_pmbcr btn btn-mini"><i class="icon-plus"></i>Tambah</span></td>
    <?php } ?>
</tr>
<?php }?>
<?php } else {?>
