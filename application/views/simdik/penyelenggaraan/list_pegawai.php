<script>
    ...
    function load(page,filter){
        ...
        if(filter!=''){
            $.get('<?php echo base_url()?>penyelenggaraan/pegawai/list_pegawai/'+page+'/'+filter, function(result){
                $('#body_table').html(result);
            });
        }else{
            $.get('<?php echo base_url()?>penyelenggaraan/pegawai/list_pegawai/'+page, function(result){
                $('#body_table').empty();
                $('#body_table').html(result);
            });
        }
