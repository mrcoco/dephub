<script>
    ...
    function load(page,filter){
        ...
        if(filter!=''){
            $.get('<?php echo base_url()?>elibrary/admin/list_user_ajax/'+page+'/'+filter, function(result){
                $('#body_table').html(result);
            });
        }else{
            $.get('<?php echo base_url()?>elibrary/admin/list_user_ajax/'+page, function(result){
                $('#body_table').empty();
                $('#body_table').html(result);
            });
        }
