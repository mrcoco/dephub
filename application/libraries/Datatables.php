...
class Datatables {
    ...
    public function group_by($column) {
        ...
        $this->ci->db->_count_string = "SELECT COUNT(DISTINCT $column) AS ";
        return $this;
    }
     ...
     *
     * @param string $key_condition
     * @param string $val
     * @param bool $backtick_protect
     * @return mixed
     */
    public function like($key_condition, $val, $backtick_protect = FALSE) {
    ...
    public function generate2($charset = 'UTF-8') {
        $this->get_paging();
        $this->get_ordering();
        $this->get_filtering();
        return $this->produce_output2($charset);
    }
    ...
    protected function produce_output2($charset) {
        $aaData = array();
        $rResult = $this->get_display_result();
        $iTotal = $this->get_total_results();
        $iFilteredTotal = $this->get_total_results(TRUE);
    ...
    protected function get_total_results($filtering = FALSE) {
        ...
        foreach ($this->where as $key => $val)
            $this->ci->db->$key($val[0], $val[1], $val[2]);
